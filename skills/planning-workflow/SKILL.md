...
수동 완성도 체크리스트를 사용자에게 제시한다 (reviewer가 자동 검증할 수 없는 항목):
- [ ] 모든 기능이 경계가 분명한 책임을 갖는가 (도메인 맥락에 의존하는 주관적 판단)
- [ ] 화면 경계가 사용자 목표 기준으로 분리되었는가 (하나의 화면에 여러 목표가 혼재하지 않는가)
- [ ] 인수조건이 사용자가 검토 가능한 수준으로 구체적인가 (형식은 S10이 검증하지만, 내용의 충분성은 사용자만 판단 가능)
- [ ] intake의 핵심 행동이 명세에 충분히 반영되었는가
- [ ] intake의 화면 구성(사이드바, 패널, 탭 등)이 레이아웃에 반영되었는가
- [ ] intake의 특수 인터랙션(드래그앤드롭, 인라인 편집 등)이 명세에 반영되었는가
- [ ] intake의 제약사항이 명세에 반영되었는가

체크리스트와 열린 이슈를 함께 보여주고 **항상** 사용자 확인을 받는다. 이슈 유무와 관계없이 사용자가 "진행"을 선택해야 6단계로 넘어간다.
...
열린 이슈가 있는 상태에서 `진행`을 선택하면, reviewer가 S1~S12에서 FAIL 가능한 구조적 결함이 포함되어 있을 경우 경고한다: "이 이슈는 reviewer가 FAIL 처리할 가능성이 높습니다. 그래도 진행하시겠습니까?"
...
2. reviewer가 반환한 개별 항목 결과를 집계하여 하네스가 PASS/FAIL을 산출한다:
...
- S11: intake 결정적 필드 3개(화면 목적 → purpose, viewport → viewport, 모달 → 레이아웃 모달 항목)를 자동 대조. 나머지 4개(핵심 행동, 화면 구성, 특수 인터랙션, 제약사항)는 5단계 수동 체크리스트에서 사용자가 판단
- S12: `viewport: [pc, mobile]`이면 뷰포트별 레이아웃 섹션(`### 레이아웃 (PC)`, `### 레이아웃 (Mobile)`)이 존재하는지
...
수동 체크리스트 (5단계에서 사용자가 직접 판단한 항목 — reviewer는 자동 검증 불가):
...
2. reviewer가 반환한 개별 항목 결과를 집계하여 하네스가 PASS/FAIL을 산출한다:
   ...
   - **FAIL** (fail 항목 1개 이상 또는 필수 항목 skip 1개 이상) → 상태를 `wireframe_review`로 유지하고:
     ...
     3. planner가 수정 책임자인 경우: planner 수정 → **영향 범위 spec-review 재실행**(수정된 화면의 screen spec + 관련 feature 파일 + INDEX.md) → spec-review PASS 확인 → **wireframer 재생성** → wireframe-review 재호출. spec-review가 재실패하면 `wireframe_review` 상태를 유지한 채 planner 재수정을 위임한다 (3회 카운트에 포함). wireframer가 수정 책임자인 경우: wireframer 수정 → wireframe-review 재호출
...
**skip 분류:**
- **필수 항목의 skip = block**: S1~S11, W1~W11은 필수 항목이다. skip이면 선행 파일 또는 섹션이 누락된 것이므로 fail과 동일하게 처리한다 (spec-review: `needs_revision` 전이, wireframe-review: `wireframe_review` 유지). 책임 라우팅 표에 따라 planner 또는 wireframer에게 보완을 위임한다. W12는 모든 와이어프레임 파일에 적용된다
- **조건부 항목의 skip = 정상**: S12는 단일 뷰포트일 때 skip이 정상이다. 다중 뷰포트인데 skip이면 block(= `needs_revision` 전이)으로 처리한다
...
reviewer에게 위임할 때, 1단계 수집 정보의 명세 반영 여부도 함께 확인을 요청한다 (reviewer S11).
...
| S10 인수조건 형식 | planner | |
| S11 수집 정보 반영 | planner | intake 내용이 명세에 미반영 |
| S12 뷰포트별 레이아웃 | planner | 다중 뷰포트인데 레이아웃 섹션 분리 누락 |
...
- `pending_approval` — 5단계 완료 후 항상 진입. 수동 완성도 체크리스트와 열린 이슈를 사용자에게 제시하고 승인 대기
...
| 인수조건이 Given/When/Then 형식을 따르지 않거나 모호한 표현이 포함됐을 때 | reviewer (S10) | 하네스 | 형식·금칙어 자동 검출. 내용의 충분성은 5단계 수동 체크 |
| 같은 책임이 여러 기능에 중복될 때 | 사용자 (5단계 수동 체크) | 사용자 | 도메인 맥락 필요, 자동 검증 불가 |
| 열린 이슈를 안고 진행할지 여부 | 하네스 (5단계 요약) | 사용자 | 사용자가 "진행" 선택 시에만 허용 |
| 정책 미확정 상태에서 다음 단계로 갈지 여부 | 사용자 (5단계 수동 체크) | 사용자 | reviewer FAIL 가능성 경고 후 사용자 판단 |
