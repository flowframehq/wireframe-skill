...
| `상태` | 상태 정의 테이블 | Planner + Designer |
| `인터랙션` | 사용자 인터랙션 목록 | Planner + Designer |
| `비즈니스 로직` | 비즈니스 규칙 (와이어프레임 미반영) | Planner + Dev |
...
| 필드 | 필수 | 설명 |
|------|------|------|
| `screenId` | Yes | 폴더명과 동일 (UPPER_KEBAB, 번호 없음). 파일명에는 소문자로 사용 |
| `title` | Yes | 화면 제목 (한국어) |
